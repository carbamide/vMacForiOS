...
- (void)_createKeyboardView {
    ...
    keyboardView.searchPaths = [kAppDelegate searchPaths];
    keyboardView.layout = [defaults objectForKey:@"KeyboardLayout"];
    [self addSubview:keyboardView];
    keyboardView.delegate = [VirtualMouseController sharedInstance];
}
...
- (void)_createInsertDiskView {
    ...
    insertDiskView.diskDrive = [VirtualDiskDriveController sharedInstance];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    if ([[event allTouches] count] > 1) {
        ...
        if (trackpadMode) {
            ...
            [kAppDelegate cancelPreviousPerformRequestsWithTarget:kAppDelegate selector:@selector(setMouseButtonDown) object:nil];
        }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (trackpadMode) {
        if (trackpadClick && (mouseDiff <= TRACKPAD_CLICK_TIME)) [self scheduleMouseClickAt:[[VirtualMouseController sharedInstance] mouseLoc]];
        trackpadClick = NO;
        if (mouseDrag) {
            [[VirtualMouseController sharedInstance] setMouseButtonUp];
            mouseDrag = NO;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (trackpadMode) {
        ...
        locDiff.v *= accel;
        trackpadClick = NO;
        [[VirtualMouseController sharedInstance] moveMouse:locDiff button:mouseDrag];
    } else {
        if (!mouseDrag) {
            // start dragging, mouseDown at current position NOW
            [[VirtualMouseController sharedInstance] cancelPreviousPerformRequestsWithTarget:[VirtualMouseController sharedInstance] selector:@selector(setMouseButtonDown) object:nil];
...
- (void)mouseClick {
    clickScheduled = NO;
    if (mouseDrag) return;
    [[VirtualMouseController sharedInstance] setMouseLoc:clickLoc button:YES];
    DoEmulateOneTick();
    DoEmulateOneTick();
    [[VirtualMouseController sharedInstance] setMouseButtonUp];
    CurEmulatedTime += 2;
...
- (void)scrollScreenViewTo:(Direction)scroll {
    ...
    CGRect screenFrame = screenView.frame;
    if (scroll & dirDown) screenFrame.origin.y = ([kAppDelegate isRetinaDisplay]?320.5:320.0)-vMacScreenHeight;
